...
namespace Baikal
{
	...
	{
	public:
		ClwMultipleOutput(std::uint32_t w, std::uint32_t h, CLWContext context)
			: Output(w, h)
			, m_context(context)
			, m_data(context.CreateBuffer<RadeonRays::float3>(w*h*MULTIPLE_VIEW_SIZE, CL_MEM_READ_WRITE))
		{
		}
